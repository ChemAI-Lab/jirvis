import pdb
import pytorch_lightning as pl
import abc
import hydra
import torch.optim.lr_scheduler
import warnings
from jirvis.train.core.data import datasets


class BaseSystem(pl.LightningModule, abc.ABC):
    def __init__(self, cfg):
        super(BaseSystem, self).__init__()
        if cfg.data.name == "nmr_enc":
            assert (
                cfg.system.model.arch.vocab_size is not None
            ), f"Updating vocab size failed: {cfg.system.model.arch.vocab_size}"

        # when save hyperparameters, the self.task will be ignored
        self.save_hyperparameters()
        data_cfg = cfg.data
        self.automatic_optimization = True
        self.config = cfg.system
        self.train_cfg = self.config.train
        self.model_cfg = self.config.model
        self.model = self.build_model()
        self.loss_func = self.build_loss_func()
        self.data_transform = self.build_data_transform()
        self.build_datamodule(data_cfg)

    def build_datamodule(self, data_cfg, **kwargs):
        self.datamodule = datasets[data_cfg.name](data_cfg.data, system_cfg=self.config)

    def get_datamodule(self):
        return self.datamodule

    def build_data_transform(self):
        if "data_transform" in self.config and self.config.data_transform is not None:
            return hydra.utils.instantiate(self.config.data_transform)
        else:
            return None

    def build_trainer(self):
        trainer = hydra.utils.instantiate(self.train_cfg.trainer)
        pdb.set_trace()
        return trainer

    def training_step(self, batch, batch_idx, **kwargs):
        optimizer = self.optimizers()
        loss = self.forward(batch, **kwargs)
        optimizer.zero_grad()
        self.manual_backward(loss)
        optimizer.step()

        if hasattr(self, "lr_scheduler"):
            self.lr_scheduler.step()

        return {"loss": loss}

    def build_model(self, **kwargs):
        model = hydra.utils.instantiate(self.model_cfg.arch)
        return model

    def build_loss_func(self):
        if "loss_func" in self.train_cfg:
            loss_func = hydra.utils.instantiate(self.train_cfg.loss_func)
            return loss_func
        else:
            warnings.warn("No loss function is specified, using default loss function")

    def configure_optimizers(self, **kwargs):
        optimizer = hydra.utils.instantiate(
            self.train_cfg.optimizer, self.model.parameters()
        )

        if getattr(self.train_cfg, "lr_scheduler", None) is not None:
            sched_cfgs = self.train_cfg.lr_scheduler.schedulers
            warmup = hydra.utils.instantiate(sched_cfgs[0], optimizer=optimizer)
            cosine = hydra.utils.instantiate(sched_cfgs[1], optimizer=optimizer)
            milestones = self.train_cfg.lr_scheduler.milestones

            scheduler = torch.optim.lr_scheduler.SequentialLR(
                optimizer, schedulers=[warmup, cosine], milestones=milestones
            )

            return {
                "optimizer": optimizer,
                "lr_scheduler": {
                    "scheduler": scheduler,
                    "interval": "epoch",  # step each epoch
                },
            }

        return optimizer

    def validation_step(self, batch, batch_idx, **kwargs):
        # TODO using task layer
        pass

    @abc.abstractmethod
    def forward(self, x, **kwargs):
        raise NotImplementedError

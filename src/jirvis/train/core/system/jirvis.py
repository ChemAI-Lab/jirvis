...
class jirvis(BaseSystem):
    def __init__(self, config, **kwargs):
        # Calculate dynamic num_labels based on selected label indices BEFORE parent init
        if hasattr(config.system, "data") and hasattr(
            config.system.data, "label_indices"
        ):
            selected_labels_count = len(config.system.data.label_indices)
            # Add 1 for catch-all label if use_catch_all is enabled
            use_catch_all = getattr(config.data.data, "use_catch_all", False)
            num_labels = selected_labels_count + (1 if use_catch_all else 0)
            # Update the config with the calculated num_labels
            config.system.model.arch.num_labels = num_labels
        else:

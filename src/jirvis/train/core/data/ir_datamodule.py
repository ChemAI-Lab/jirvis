...
class IRData(DataBase):
    def __init__(self, cfg, system_cfg=None, **kwargs):
        ...
        # Get label indices from system config if available
        if (
            system_cfg
            and hasattr(system_cfg, "data")
            and hasattr(system_cfg.data, "label_indices")
        ):
            self.label_indices = system_cfg.data.label_indices
        else:
            self.label_indices = list(range(24))  # default fallback

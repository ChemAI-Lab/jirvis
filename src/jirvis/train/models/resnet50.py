...
class jirvis(nn.Module):
    def __init__(
        ...
        self.model = ResNetFeatureExtractor(
            num_labels, model_name, model_type, channels, pretrained
        )
...
class ResNetFeatureExtractor(nn.Module):
    def __init__(self, num_labels, model_name, model_type, channels, pretrained=True):
        super().__init__()

        # Get ResNet model
        resnet_map = {
            "R18": (models.resnet18, ResNet18_Weights.IMAGENET1K_V1),
            "R34": (models.resnet34, ResNet34_Weights.IMAGENET1K_V1),
        ...
        # Use pretrained weights or start from scratch
        if pretrained:
        ...
        feature_dims = {"R18": 512, "R34": 512, "R50": 2048, "R101": 2048, "R152": 2048}
        feature_dim = feature_dims[model_name]

        # Get MLP based on type - adapt to feature dimension
        if feature_dim == 512:  # ResNet18/34
            mlp_configs = {
                "standard": [(512, 256, 0.4)],
                "3_layer": [(512, 256, 0.3), (256, 128, 0.3)],
                "squeeze": [(512, 128, 0.3), (128, 64, 0.3)],
                "4_layer": [(512, 256, 0.3), (256, 128, 0.3), (128, 64, 0.3)],
            }
        ...
        if model_type == "residual":
            if feature_dim == 512:  # ResNet18/34
                self.mlp = nn.Sequential(
                    ...
                    ResidualMLPBlock(256, 0.3),
                    self._make_layer(256, 128, 0.3),
